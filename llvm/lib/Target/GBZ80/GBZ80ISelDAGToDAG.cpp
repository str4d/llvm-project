//===-- GBZ80ISelDAGToDAG.cpp - A dag to dag inst selector for GBZ80 ----------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines an instruction selector for the GBZ80 target.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "GBZ80-isel"

namespace llvm {

/// Lowers LLVM IR (in DAG form) to GBZ80 MC instructions (in DAG form).
...
public:
  ...
  StringRef getPassName() const override {
    return "GBZ80 DAG->DAG Instruction Selection";
  }

  bool runOnMachineFunction(MachineFunction &MF) override;

  bool SelectIAddr(SDNode *Op, SDValue N, SDValue &Address);
  bool SelectHRAMRegAddr(SDNode *Op, SDValue N, SDValue &Reg);
...
private:
  ...
  const GBZ80Subtarget *Subtarget;
};

bool GBZ80DAGToDAGISel::runOnMachineFunction(MachineFunction &MF) {
  Subtarget = &MF.getSubtarget<GBZ80Subtarget>();
  return SelectionDAGISel::runOnMachineFunction(MF);
}

#if 0
bool GBZ80DAGToDAGISel::SelectIAddr(SDNode *Op, SDValue N, SDValue &Address) {
  SDLoc dl(Op);
  auto DL = CurDAG->getDataLayout();
  MVT PtrVT = getTargetLowering()->getPointerTy(DL);

  // Match immediates or globals.
  if (const GlobalAddressSDNode *GA = dyn_cast<GlobalAddressSDNode>(N)) {
    Address = CurDAG->getTargetGlobalAddress(GA->getGlobal(), ;
  }
...
bool GBZ80DAGToDAGISel::SelectHRAMRegAddr(SDNode *Op, SDValue N, SDValue &Reg) {
  SDLoc dl(Op);
  auto DL = CurDAG->getDataLayout();
...
#if 0
// We don't need this for now. if ever
unsigned GBZ80DAGToDAGISel::selectIndexedProgMemLoad(const LoadSDNode *LD,
                                                   MVT VT) {
...
bool GBZ80DAGToDAGISel::SelectInlineAsmMemoryOperand(const SDValue &Op,
  ...
  if (RegNode &&
      RI.getRegClass(RegNode->getReg()) == &GB::PTRDISPREGSRegClass) {
    OutOps.push_back(Op);
    return false;
  }
  ...
  if (Op->getOpcode() == ISD::ADD || Op->getOpcode() == ISD::SUB) {
    ...
    if (CopyFromRegOp->getOpcode() == ISD::CopyFromReg) {
      ...
      CanHandleRegImmOpt &= (TargetRegisterInfo::isVirtualRegister(Reg) ||
                             GB::PTRDISPREGSRegClass.contains(Reg));
    } else {
    ...
    if (CanHandleRegImmOpt) {
      ...
      if (RI.getRegClass(Reg) != &GB::PTRDISPREGSRegClass) {
        SDLoc dl(CopyFromRegOp);

        unsigned VReg = RI.createVirtualRegister(&GB::PTRDISPREGSRegClass);
...
template <> bool GBZ80DAGToDAGISel::select<ISD::STORE>(SDNode *N) {
  ...
  return false;
  assert(false && "not implemented, check this");
#if 0
  ...
  unsigned Opc = (VT == MVT::i16) ? GB::STDWSPQRr : GB::STDSPQRr;
...
template <> bool GBZ80DAGToDAGISel::select<GBISD::CALL>(SDNode *N) {
  SDValue InFlag;
  ...
  SDLoc DL(N);
  Chain = CurDAG->getCopyToReg(Chain, DL, GB::R31R30, Callee, InFlag);
  SmallVector<SDValue, 8> Ops;
  Ops.push_back(CurDAG->getRegister(GB::R31R30, MVT::i16));
  ...
  SDNode *ResNode =
    CurDAG->getMachineNode(GB::CALL_nn, DL, MVT::Other, MVT::Glue, Ops);
...
template <> bool GBZ80DAGToDAGISel::select<ISD::BRIND>(SDNode *N) {
  SDValue Chain = N->getOperand(0);
  SDValue JmpAddr = N->getOperand(1);
  assert(false && "brind not implemented");
#if 0
  SDLoc DL(N);
  // Move the destination address of the indirect branch into R31R30.
  Chain = CurDAG->getCopyToReg(Chain, DL, GB::R31R30, JmpAddr);
  SDNode *ResNode = CurDAG->getMachineNode(GB::IJMP, DL, MVT::Other, Chain);
...
template <> bool GBZ80DAGToDAGISel::select<ISD::LOAD>(SDNode *N) {
  const LoadSDNode *LD = cast<LoadSDNode>(N);
  ISD::MemIndexedMode AM = LD->getAddressingMode();
  MVT VT = LD->getMemoryVT().getSimpleVT();
  auto PtrVT = getTargetLowering()->getPointerTy(CurDAG->getDataLayout());

  // We only care if this load uses a POSTINC or POSTDEC mode.
  if ((LD->getExtensionType() != ISD::NON_EXTLOAD) ||
    (AM != ISD::POST_INC && AM != ISD::POST_DEC))
    return false;

  if (VT.SimpleTy != MVT::i8)
    return false;

  bool isDec = (AM == ISD::POST_DEC);
  unsigned Opcode = isDec ? GB::LD8_DEC : GB::LD8_INC;
  int Offs = cast<ConstantSDNode>(LD->getOffset())->getSExtValue();
...
bool GBZ80DAGToDAGISel::selectMultiplication(llvm::SDNode *N) {
  SDLoc DL(N);
  ...
  unsigned MachineOp = isSigned ? GB::MULSRdRr : GB::MULRdRr;
  ...
  if (N->hasAnyUseOfValue(0)) {
    SDValue CopyFromLo =
        CurDAG->getCopyFromReg(InChain, DL, GB::R0, Type, InGlue);
  ...
  if (N->hasAnyUseOfValue(1)) {
    SDValue CopyFromHi =
        CurDAG->getCopyFromReg(InChain, DL, GB::R1, Type, InGlue);
...
void GBZ80DAGToDAGISel::Select(SDNode *N) {
...
bool GBZ80DAGToDAGISel::trySelect(SDNode *N) {
  ...
  case GBISD::CALL: return select<GBISD::CALL>(N);
